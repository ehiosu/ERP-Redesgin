  ...
  return (
    <main   className='w-full   h-screen    bg-[#E0F4FF]  relative  after:w-[100%]   after:h-[150%] after:absolute  after:z-[5]   after:rounded-[123rem]  after:bg-white after:-top-[25%] after:-left-[15%]   overflow-hidden'>
        <div className="absolute    w-full  h-full  top-0   left-0  z-10    p-2">
          <div className="flex justify-start h-24   items-start  ">
         ...
         <p className='xl:text-[3rem] md:text-[2.5rem]   w-full md:w-[90%] font-[600]    text-[2rem]'    >Streamline Your Business   <br /> Unleash Your Potential  <br /> with <span    className='text-[#1B49C1]   '>Circle ERP</span></p>
         <p className='lg:text-[1.15rem]  md:text-[1.3rem]  font-semibold   mt-[1rem]       w-[90%] '>A comprehensive solution for managing various aspects of business operations to drive organizational success</p>
         <p className='text-[0.85rem]   mt-[1rem]   w-[90%] '>Operate more efficiently, improve decision-making, and achieve strategic objectives with Circle ERP.</p>
         </div>
    ...
    return(
        <div    className={`h-[80%] flex-1   xl:w-60 md:w-60   rounded-2xl  p-4  w-40  ${data.style}  flex flex-col    justify-between shadow-md sm:w-[32rem]`  }>
           <div className='text-[2.5rem]    font-bold   text-white'> {data.icon}</div>
            <p  className='mt-auto  text-[1.2rem] font-bold  border-b-2   border-b-white/50       text-white p-1'>{data.title}</p>
        </div>
    )
}